...
<head>
    ...
    <style>
        ...
        div:where(.swal2-container) button:where(.swal2-styled).swal2-confirm { background-color: #85A385 !important; }
    </style>
</head>
<body class="text-white flex flex-col min-h-screen">
        ...
        <div class="book-card grid lg:grid-cols-[1fr_2fr] gap-12 lg:gap-0 p-8 md:p-12 rounded-[2rem] w-full shadow-2xl relative" style="background-color: #85A385;">
            ...
            <div class="lg:pl-16 flex flex-col justify-center mt-8 lg:mt-0 z-0">
                ...
                <div class="flex flex-col gap-4 mt-auto">
                    ...
                    @else
                        {{-- LOGIKA TOMBOL PINJAM CEK STOK & JAVASCRIPT MANUAL SUBMIT --}}
                        @if(isset($buku) && $buku->Stok > 0)
                            <form id="formPinjam" action="{{ Route::has('peminjaman.store') ? route('peminjaman.store', $buku->BukuID ?? 0) : '#' }}" method="POST" class="m-0 bg-white/10 p-5 rounded-2xl border border-white/20">
                                @csrf
                                <p class="text-xs font-bold uppercase tracking-widest text-white/90 mb-3">Pilih Rencana Tanggal Kembali:</p>
                                
                                <div class="flex flex-col sm:flex-row gap-4">
                                    <input type="date" id="inputTanggal" name="BatasWaktu" required 
                                    ...
                                    {{-- PERUBAHAN: type="submit" jadi type="button" biar gak nyelonong --}}
                                    <button type="button" onclick="validasiPinjam()" class="bg-[#6B8E6B] text-white border border-white/30 hover:bg-[#5C805C] px-8 py-3 rounded-xl font-bold text-sm transition-all shadow-md transform hover:-translate-y-1 whitespace-nowrap">
                                        Pinjam Buku
                                    </button>
                                </div>
                    ...
                    @if(isset($buku))
                        <div class="flex gap-4 mt-2">
                            <form action="{{ route('koleksi.store', $buku->BukuID) }}" method="POST" class="m-0 w-full sm:w-auto">
                                @csrf
                                <button type="submit" class="w-full sm:w-auto bg-[#85A385] border border-white/30 text-white hover:bg-[#6B8E6B] px-6 py-2.5 rounded-lg font-bold text-sm transition-all shadow-md transform hover:-translate-y-1">
                                    Save ke Koleksi
                                </button>
                            </form>
                        </div>
    ...
    {{-- LOGIKA JAVASCRIPT MANUAL BUAT CEK ALAMAT --}}
    <script>
        function validasiPinjam() {
            // Tangkep elemen form, tanggal, dan data alamat dari database
            let form = document.getElementById('formPinjam');
            let tanggal = document.getElementById('inputTanggal').value;
            let userAlamat = {!! json_encode(auth()->check() ? auth()->user()->Alamat : '') !!};

            // 1. Cek Kalender Dulu (Harus diisi)
            if (!tanggal) {
                form.reportValidity(); // Munculin peringatan bawaan browser suruh isi kalender
                return;
            }

            // 2. Cek Alamat (Kalau kosong, munculin Pop-up!)
            if (!userAlamat || userAlamat.trim() === "") {
                Swal.fire({
                    icon: 'warning',
                    title: 'Oops, Tunggu Dulu!',
                    text: 'Lu wajib melengkapi Alamat Rumah di menu Profil sebelum bisa meminjam buku ya bro!',
                    confirmButtonText: 'Isi Alamat Sekarang',
                ...
                // 3. Kalau tanggal udah diisi dan alamat aman, baru deh datanya dikirim ke server!
                form.submit();
            }
        }

        // TAMPILIN NOTIFIKASI ERROR DARI CONTROLLER (Jaga-jaga kalau ada error lain)
        @if(session('error'))
            Swal.fire({
                icon: 'error',
                title: 'Gagal',
                text: "{{ session('error') }}",
                confirmButtonColor: '#85A385'
            });
        @endif

        // TAMPILIN NOTIFIKASI SUKSES PINJAM
        @if(session('success'))
            Swal.fire({
                icon: 'success',
                title: 'Berhasil!',
